...
/**
 * Clase que procesa los eventos en la RED, en este contexto corresponden a los mensajes enviados.
 *
 */
	...
	public void processEvent(Node myNode, int layerId, Object event) {
		...
		if(message.getType()==0){
			...
			if(myNode.getID()== message.getDestination()){
				...
				message.setType(1);
				int idNode = message.removeToCamino();			
			...
			//Si no soy el destino, pero tengo en la caché guardada la query	
				...
				message.addToCamino((int)myNode.getID());
				sendmessage(myNode, layerId, message, idNode);
				getStats();
			}
